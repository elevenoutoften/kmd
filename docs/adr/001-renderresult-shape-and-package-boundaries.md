...
The current `ParseResult` interface is the initial public API, renamed to `RenderResult`:
...
| Optional design | `kmd-web/design` | No | No | No |
...
See the North Star (§1-2) for the complete layer model and package structure.
...
See the North Star (§3) for the complete capability contract definitions.
...
See the North Star (§4) for the complete export map.

### 5. Package boundaries

Single `kmd-web` package with subpath exports. See North Star (§2) for the complete table.
...
| Capability contracts too narrow for future features | Design as extensible interfaces with optional methods; add fields in minor versions |
...
- The `ios-security.test.ts` tests (kmd-ios only) partially cover this by testing that script elements are stripped, but do not test Mermaid SVG output specifically. Add a test that renders a Mermaid diagram and verifies no `<script>` tags survive in the output.

...
4. Ran the baseline commands recorded individually in §8: tests and TypeScript
   compilation for both repositories, plus the desktop frontend production build.
...
| `rehype-shiki.ts` | **Optional highlighting** | Pure rehype integration, but Shiki is a heavy optional dependency and belongs behind the highlighting feature boundary. |
| `rehype-mermaid.ts` (rehype plugin) | **Optional Mermaid** | Pure HAST transform, but owned by the optional Mermaid feature. It must be split from the browser render function. |
...
| `lazy-katex-css.ts` | **Optional math/browser integration** | Lazy CSS import is browser/bundler-facing and belongs behind the optional math feature boundary. |
...
| Shiki highlighting (rehype plugin) | `kmd-web/highlighting` | Heavy optional integration; no baseline static import |
| Mermaid rehype plugin (tree transform) | `kmd-web/mermaid` | Pure transform, but owned by the optional Mermaid feature |
| Mermaid render (`renderMermaidPlaceholders`) | `kmd-web/mermaid` + browser runtime | Requires DOM and the sanitized-SVG insertion boundary |
| KaTeX transform and CSS loading | `kmd-web/math` | Optional math feature; CSS loading is browser-facing |
...
- Consumers must apply a restrictive CSP as defense in depth; CSP does not replace generated-SVG sanitization.
...
- Before publication, pass Mermaid output through a dedicated strict SVG sanitization boundary that strips scripts, event handlers, external references, unsafe URLs, and unsupported elements/attributes before insertion.
- Add malicious Mermaid/SVG fixtures covering scripts, event handlers, external references, unsafe URLs, and active embedding elements.
